...
The improvement loop is methodological. The governance loop is teleological. Both are essential, but they are answers to different questions, done at different cadences, by people with different decision rights. Conflating them --- treating "add a gate" and "change the goal" as the same kind of activity --- obscures where human judgment is most needed. The deepest layer of the governance loop cannot be automated, because that is where intent itself is defined, and bare intent is not computable from data.
...
An agent can generate ten files in two minutes. A human cannot review, understand, and reorient at that pace. The binding constraint in the framework is not validation latency or shipping speed. It is the **cognitive absorption rate** --- the speed at which a human can comprehend what the agent built, what it implies about the problem, and whether the goal should change in response. The governance loop is slow not because governance is inherently slow, but because the comprehension it depends on is. And that comprehension --- the formation of fresh judgment about something just built --- cannot be handed off, because it is where judgment meets intent.
...
A note on vocabulary, because one word is already straining under the load. Throughout the discourse --- and in the first draft of this essay --- *harness* is used to name everything: the linter bolted to the model, the test runner, the approval gates, the goal-setting structure, the whole multi-level apparatus. That is precisely the overloading this section set out to avoid, committed one level up. The fix is the same as before: split the word.

Let **harness** mean the deterministic machinery wrapped directly around the model --- the tools it can invoke and the gates that verify its output. The harness serves execution and validation. It is close to the metal: a compiler, a linter, a type checker, a sandboxed test runner, a permission boundary enforced at the engine level. It is computational, fast, and binary --- the part of the system you could, in principle, prove correct.

Let **scaffold** mean the organisational system that surrounds the harness --- the retrospectives that decide which gates to add, the goals that define what "done" means, the trajectory reviews where a human absorbs what was built and decides whether the goal should change. The scaffold serves improvement and governance. It is not close to the metal; it is close to intent. It is human and agentic, slow, and judgment-laden --- the part of the system no one can prove correct, only judge.

The harness wraps a model. The scaffold wraps the harness --- and, at higher levels of the fractal, wraps the teams and value chains that run many harnesses at once. Engineers iterate on harnesses. Organisations iterate on scaffolds. Keeping the two words apart keeps the two activities apart, which is the entire point of a vocabulary.
...
| Scaffold as shared infrastructure across the engineering org | Implicit scaffold that lived in individual engineers' heads | Governance --- the definition of what "good" means --- must be explicit and shared to scale beyond a single developer. |
...
| What worked | What didn't | Insight |
|---|---|---|
| Solo developer + agent loop at high velocity | Rolling the same workflow to a team without shared scaffold | Governance: the solo developer's scaffold was *implicit* --- it lived in his head. Brains do not compose. |
...
The four purposes are not unique to any one level. They recur at every organisational scale, at progressively slower cadences --- the delivery team, the enterprise, and the market each run all four. The structure is self-similar. And if the same four loops repeat at every level, the levels must connect to one another --- which they do, in a specific and consequential way.
...
Each level receives a governance decision from above, executes against it, validates the output, improves its process, and makes governance decisions for the level below. The cascade is how intent flows downward through an organisation. The harness is what governance at one level looks like once it has been encoded as execution constraints for the level below.
...
This is [Kief Morris](https://martinfowler.com/articles/exploring-gen-ai/humans-and-agents.html)'s "humans on the loop" placed into the framework. The human reviews the system that wraps the model --- the *harness* of tools, gates, and guardrails, and the *scaffold* of goals and retrospectives around it. When output is wrong, the human does not fix the output. They strengthen the harness. When strategy shifts, the human does not re-prompt the agent. They change the governance --- the goal, the acceptance criteria, the definition of done.
...
The governance loop is where human judgment resides in an agentic organisation. Its deepest layer cannot be automated --- not because the technology is insufficient, but because that layer is where *intent* is defined and *comprehension* is exercised, and neither is computable from data. Much of what we loosely call governance sits above that layer and is instrumental --- derivable from a higher goal plus data --- and that part will increasingly be automated; the distinction is drawn out below. But the floor remains. You cannot optimise your way to a purpose. And while you can accelerate the *acquisition* of understanding --- AI has done exactly that, and spectacularly --- you cannot yet outsource the *formation of judgment* about a thing you have only just made.
...
### Building the Harness Is Governance

Assembling the harness --- choosing which deterministic gates to enforce, setting the goals and acceptance criteria the agent must meet, placing human-approval boundaries at dangerous thresholds, requiring trajectory logging for auditability --- is not a software task. It is the scaffold's defining act, and it is *governance*. It happened to be discovered in software because software is the domain where execution was accelerated first.

Every domain will need its own harness, and the governance to build one. Law will need deterministic gates for legal reasoning --- does the brief cite precedent correctly? Does it address the relevant statutes? Medicine will need deterministic gates for diagnosis --- does the proposed treatment align with established guidelines? Does it account for contraindications? Finance will need deterministic gates for modelling --- does the model pass backtesting? Does it comply with regulatory constraints?

The domains that learn to build these harnesses first will be the domains where AI adoption is fastest and safest. The domains that do not will oscillate between two failure modes: rejecting AI entirely (and falling behind) or adopting AI without governance (and producing unreliable output at scale). Building the harness is the middle path, and it is the only path that scales.

### Most Governance Is Instrumental

The improvement loop and the governance loop are easy to confuse in practice, and the confusion is expensive. Adding a gate is improvement; changing the goal is governance. An organisation that treats every failure as an improvement problem --- *"let's add another gate"* --- will eventually build a harness so constraining that the agent cannot execute. An organisation that treats every failure as a governance problem --- *"maybe we're building the wrong thing"* --- will never stabilise long enough to ship. The art is knowing which loop you are in, and when to switch.

But even *governance* is not one thing, and there is a tempting overstatement lurking in this framework that honesty requires naming. To say flatly that "governance cannot be automated" is to repeat, one level up, the very overloading this essay set out to expose. *Governance* splits, just as *loops* did.

Most of what the framework calls governance is **instrumental**: a goal derived from a higher goal plus data. "Enter the European market" is not a sovereign act of human will --- it is a conclusion that should fall out of total addressable market, margin structure, competitive density, and regulatory cost. "Prioritise the consent feature" is a scheduling decision derivable from risk exposure and dependency order. These set goals for the level below, and so they are governance --- but they are *computable* governance, and as the context available to agents deepens, they will increasingly be proposed, and then made, by agents. An agent with the right data may well make the European call better than a biased executive defending last year's strategy.

What does not reduce to data is **terminal** governance: the choice of what to value in the first place. Why enter any market. What the enterprise is *for*. Which goods count as goods. There is no higher goal from which to derive this one --- it is where the regress of goals stops. *This* is what cannot be optimised, because optimisation presupposes an objective function, and terminal governance is the act of choosing the objective function itself.

The honest trajectory, then, is not the comfortable "humans keep governance." It is narrower and more unsettling: the human-owned region contracts over time --- from all governance, to instrumental governance, to terminal governance alone. The instrumental layer drains upward into the agents. What remains is the answer to *why do any of this at all* --- and the genuinely open question is not whether that layer survives, but how few people a civilisation needs to hold it, and who they turn out to be.

One caution against filing this under "future problem": terminal choices are load-bearing *now*, not merely eventually. The data that would justify entering Europe is itself shaped by prior terminal commitments --- what the organisation chose to measure, what it counts as success, whose welfare even enters the model. Instrumental governance always runs on rails that terminal governance laid down. The rails are invisible, which is precisely why they are so easy to mistake for nothing.

And laying those rails depends on comprehension --- you cannot govern what you have not absorbed. The loops that matter most are the ones that cannot be automated.

---

## Conclusion: The Harness and the Scaffold
...
The harness pattern is not about software. It is about how complex human effort is organised when the speed of thought is no longer the binding constraint. The domains that understand this --- that distinguish execution from validation from improvement from governance, that build a harness and a scaffold at each level, that recognise the fractal and cascade properties of the loops they inhabit, and that design for the cognitive absorption rate of the humans who must comprehend what the machines produce --- will thrive. The domains that do not will generate infinite drafts and ship none of them.

The loop is not the point. The harness and the scaffold are. The harness --- the deterministic machinery wrapped around the model --- is the artefact engineers will iterate on. The scaffold --- the organisational system that decides what the harness is for, and absorbs what it produces --- is the artefact the rest of us will iterate on. At every level, in every domain, for the next decade.
